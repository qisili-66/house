...
基于 Flask + MySQL 开发房源数据分析与个性化推荐系统，负责后端接口设计、数据模型构建、用户收藏/浏览记录管理、房源检索、推荐算法和可视化接口实现。系统通过 SQLAlchemy 完成房源聚合查询和分页展示，使用 Pearson 相似度基于用户浏览行为生成个性化推荐，并结合 ECharts 和线性回归模型实现区域房源数量、户型占比、价格走势及短期价格预测展示。
